...
Note: these flagged loci may slightly change depending on whether optical duplicates were removed (not possible for ENA data) or slight assembly differences with HybPiper  
...
Note: the tasks could also be separated for easier execution or if there were errors  
In addition, if desired, the locus tree inferences could be split up and parallelised, though this was not done here  
```s
# run alignment cleaning first
qsub -v align_dir="temp_input",realign="n",clean="y",analysis="n" /path/to/scripts/align_phylo.sh
# wait for cleaning to complete, then submit jobs for loci and concatenation
qsub -v align_dir="in_align",realign="n",clean="n",analysis="l",poly="y",concord="n" /path/to/scripts/align_phylo.sh
qsub -l ncpus=8,mem=8GB -v align_dir="in_align",realign="n",clean="n",analysis="c",concord="n" /path/to/scripts/align_phylo.sh
# after these jobs finish (c. 8 hours), run concordance analysis
qsub -v align_dir="in_align",realign="n",clean="n",analysis="n",concord="y" /path/to/scripts/align_phylo.sh
```
...
python3 /g/data/nm31/ben_anderson/scripts/remove_fastas.py "$remove_line" "${locus}.fasta"
else
...
Run the phylogenetic analysis (again, could be faster to break it into separate jobs)  
```s
# from the run2 folder
qsub -v align_dir="temp_input",realign="n",clean="y",analysis="a",poly="y",concord="y" /path/to/scripts/align_phylo.sh
```
...
If wanting to plot support values (UFB, ASTRAL pp) on the ASTRAL polytomy-collapsed tree, I visually compared the collapsed tree with the plotted concatenation and ASTRAL trees  
...
# Replication test and stochastic mapping
Following review, additional analyses were requested (stochastic mapping of character states relating to parasitism on our tree)  

We decided to also test whether uploaded data (newly generated by us, uploaded to ENA) and our analyses could be replicated easily following steps in this repository  

We downloaded our previously uploaded data (but only for the final 195 samples) and ran steps as before, though this time we were unable to remove optical duplicates during read cleaning (because ENA removes Illumina tiling information)  
As a result, slightly more reads were retained for samples sequenced by us  

HybPiper assembly was run as described above, with slight increases in recovered genes at 50% length for some samples (presumably because more read depth was present as a result of retaining optical duplicates)  

Phylogenetic analyses were run as described above, with the same loci dropped as in the primary analysis (note: using the same thresholds would have resulted in slight differences with the non-deduplicated read assemblies)  

The resulting ASTRAL tree was largely congruent with the primary analysis, with slight (poorly supported) differences within some clades that were poorly resolved in the primary analysis, as might be expected  

## Stochastic mapping
To run stochastic character mapping with phytools in R, we need to convert one of our trees (e.g. concatenation or ASTRAL tree) into an ultrametric form (tips all sampled at the present and the same distance from the root)  

Note that since we have single trees (rather than a Bayesian analysis), this mapping is not accounting for the (sometimes considerable) topological uncertainty (it doesn't appear to handle polytomies), so results should be interpreted cautiously  

One study (10.1098/rsbl.2019.0848) used treePL to convert a phylogenetic tree to a time-calibrated tree, with the resulting tree used for characater mapping in phytools  

To use treePL, we need a tree file and a configuration file specifying calibrations, analyses and input tree information, including how many characters were used to estimate the tree  

The ASTRAL tree was inferred from 318 input gene trees, but the underlying alignment was 188,571 bp (replicate run)  
Given the concatenation tree includes inferred substitutions per site in taxa that are single representatives of lineages, it may be more appropriate to use for calibrating against time, as ASTRAL is unable to estimate branch lengths subtending single samples  

First, root the `concat.treefile` tree using the `plot_trees.rmd`, but before substituting labels, output a Newick format file using the ape function `write.tree`  
```r
write.tree(tree_list[[1]], file = "concat.rooted.tre")
``` 

Create a treePL configuration file (`config.txt`)  
Choose *Brachynema* (first branching clade) and one other ingroup more deeply nested to identify the most recent common ancestor  
Use as a rough calibration the crown ages provided by Zuntini et al. for Santalales (125 to 204 Ma)  
As a first run, use the "prime" line to let the program suggest additional lines for the config file (remove on second run)  
```
treefile = concat.rooted.tre
smooth = 100
numsites = 188571
mrca = SANT K09 377655
min = SANT 125
max = SANT 204
outfile = concat.dated.tre
thorough
prime
nthreads = 8
```

Run using treePL in a Singularity container  
```s
singularity exec -H "$(pwd)" treepl.sif treePL config.txt
```

For a second run add random cross validation to estimate a good smoothing parameter  
Also add the recommended lines from priming  
```
treefile = concat.rooted.tre
smooth = 100
numsites = 188571
...
The resulting tree is `concat.dated.tre`  

Now, run stochastic mapping using phytools in R  
To set character states, we need to use a table of prior probabilities (since we have samples with unknown status or possible mixed status)  
The states are: non = not parasitic, root = root hemiparasitic, aerial = mistletoes, holo = holoparasitic  
For the samples with unknown states (Aptandraceae), give 0.50 and 0.50 to non and root  
For samples with suspected mixed state (e.g. *Dendrotrophe*), give 0.75 to the more common state, and 0.25 to the suspected other state (e.g. 0.75 aerial, 0.25 root)  

Fix the root condition (common ancestor of Santalales and *Vitis* and *Berberidopsis*) as not parasitic (pi = 1, 0, 0, 0)  
Use an All Rates Different (ARD) model, since we don't know rates of change between states, nor do we expect them to be equal to each other  

NOTE: since our sample of *Heisteria* grouped with *Olax* rather than in Erythropalaceae, it is probably misidentified  
Its parasitic status should be scored as root parasitic or 0.5 0.5 not or root in additional runs (see below)  

The character prior rates can be created in a spreadsheet, then saved as a tab-delimited file, with the first column sample number  

Run the following steps in R (choosing to do it in parallel or not)  
```r
# using ideas from Liam Revell at http://blog.phytools.org/
library(phytools)
dated_tree <- read.tree("concat.dated.tre")
input_chars <- read.csv("chars.tab", sep = "\t", header = TRUE, row.names = 1)
chars <- as.matrix(input_chars)

# first fit the model (fixing the root to non-parasitic)
fitted_model <- fitMk(dated_tree, chars, pi = c(1, 0, 0, 0), model = "ARD")
# extract the matrix
fittedQ <- matrix(NA, length(fitted_model$states), length(fitted_model$states))
fittedQ[] <- c(0, fitted_model$rates)[fitted_model$index.matrix + 1]
diag(fittedQ) <- 0
diag(fittedQ) <- -rowSums(fittedQ)
colnames(fittedQ) <- fitted_model$states
rownames(fittedQ) <- fitted_model$states

# run the analysis in parallel, using 8 cores (each doing 125 simulations, for a total of 1000)
library(parallel)
trees <- mclapply(1:8, function(n, tree, x, fixedQ) make.simmap(tree, x, Q = fixedQ, pi = c(1, 0, 0, 0), nsim = 125),
	tree = dated_tree, x = chars, fixedQ = fittedQ, mc.cores = 8L)
# combine the output after it finishes
trees <- do.call(c, trees)
if(! ("multiSimmap" %in% class(trees))) class(trees) <- c("multiSimmap", class(trees))

# OR run in serial (will take longer)
trees <- make.simmap(dated_tree, chars, model = "ARD", pi = c(1, 0, 0, 0), nsim = 1000)

# summarise
mapping_summary <- summary(trees)
mapping_summary
```

```
1000 trees with a mapped discrete character with states:
 aerial, holo, not, root

trees have 10.851 changes between states on average

changes are of the following types:
     aerial,holo aerial,not aerial,root holo,aerial holo,not holo,root
x->y           0          0           0           0        0         0
     not,aerial not,holo not,root root,aerial root,holo root,not
x->y          0    0.911    3.546         5.1     1.294        0

mean total time spent in each state is:
           aerial         holo          not         root    total
raw  2225.3570206 508.69360063 2045.7490248 4142.5946500 8922.394
prop    0.2494125   0.05701313    0.2292825    0.4642918    1.000
```

To plot the tree and the results of the mapping, we can use similar approaches as in `plot_trees.rmd`  
```r
# choose one of the trees (topology the same for all) to plot
my_tree <- trees[[1]]
...
my_cex <- 0.25
# set colours
colours <- setNames(c("#CC79A7", "#E69F00", "#009E73", "#0072B2"), sort(colnames(chars)))

# read in sample names to substitute
sample_table <- read.table("samples.tab", sep = "\t")
colnames(sample_table) <- c("ID", "label")
# substitute labels
tips <- my_tree$tip.label
new_tips <- tips
...
my_tree$tip.label <- new_tips

# start a graphics device for plotting
pdf("tree.pdf", width = my_width, height = my_height, family = "ArialMT")

# plot the base tree
plot.phylo(ladderize(my_tree, right = FALSE),
	no.margin = TRUE,
	font = 1,
	edge.width = 2,
	label.offset = max(nodeHeights(my_tree)) / 100)

# plot the pies of posterior probabilities at nodes, and the posterior at tips
nodelabels(pie = mapping_summary$ace, piecol = colours, cex = my_cex)
tiplabels(pie = mapping_summary$ace[Ntip(my_tree): nrow(mapping_summary$ace), ], piecol = colours, cex = my_cex)

# stop the graphics device
invisible(dev.off())
```

After plotting, run a second run with the likely misidentified *Heisteria* scored as root parasitic to see if it influences results  
```r
chars[rownames(chars) == "K24", ] <- c(0, 1, 0, 0)
```
...
trees have 13.669 changes between states on average

changes are of the following types:
     aerial,holo aerial,not aerial,root holo,aerial holo,not holo,root
x->y           0          0           0       0.406        0     7.564
     not,aerial not,holo not,root root,aerial root,holo root,not
x->y          0    1.046        0       4.653         0        0

mean total time spent in each state is:
           aerial         holo          not         root    total
raw  2219.1649393 1033.7411760 1484.1493354 4185.3388453 8922.394
prop    0.2487185    0.1158592    0.1663398    0.4690825    1.000
```
...
trees have 6.127 changes between states on average

changes are of the following types:
     aerial,not aerial,root not,aerial not,root root,aerial root,not
x->y          0           0          0    1.073       5.054        0

mean total time spent in each state is:
           aerial          not         root    total
raw  2224.2574724 1490.2634642 5207.8733594 8922.394
prop    0.2492893    0.1670251    0.5836856    1.000
```
